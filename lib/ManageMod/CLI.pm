...
sub not_implemented { die $log->fatalf('%s %s %s is not implemented', $_[0]->prog_name, $_[0]->command, $_[1]) }
sub saveconfig      { defined $_[0]->{saveconfig} ? 1 : undef }
sub unknown_cmd     { die $log->fatalf('Unknown command for %s: %s', $_[0]->command, $_[1]) }
